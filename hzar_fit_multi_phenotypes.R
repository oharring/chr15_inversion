...
ap <- add_argument(ap, "--region", help = "region to get data from; must be fullv2 (whole transect, including new UWBM site) or mid (cascades only)",default='fullv2')
...
distdf <- read.csv(paste0(source_dir,'/data/transect_distance_20200619.csv'))

# Load the mouse data:
mousedf <- read.csv(paste0(source_dir,'/data/adult_transect_data_with_new_sites_20200619.csv'))
mousedf_mid <- droplevels(subset(mousedf, cascades == TRUE))

# Load the soil data (already subset to 1 km, see set_up_soil_1D_pops_20200128.R':
soildf <- read.csv(paste0(source_dir,'/data/soil_summary_1DPops_20200128.csv'))
